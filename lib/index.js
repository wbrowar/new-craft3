...
function run() {
    ...
    inquirer.prompt(questions).then(function (answers) {
        chwonUser  = answers['chwonUser'] || 'REPLACE_ME';
        chwonGroup = answers['chwonGroup'] || 'REPLACE_ME';
        ...
        if (verbose) {
            ...
            _verboseExec(`npm -v`);
            _log('verbose', `getting yarn version`);
            _verboseExec(`yarn --version`);
        }
        ...
        if (!fs.existsSync(projectDir)){
            ...
            _download('https://github.com/wbrowar/WB-Starter/archive/master.zip', () => {
                ...
                exec.execSync(`mysql --user=root -e 'CREATE DATABASE IF NOT EXISTS ${ '`' + name + '`'} CHARACTER SET utf8 COLLATE utf8_general_ci'`);
                ...
                _log('title', 'Running Yarn');
                _verboseExec(`yarn`);
                _log('verbose', `yarn ran`);

                _log('title', 'Running Setup');
                _verboseExec(`gulp setup --chownuser='${chwonUser}' --chowngroup='${chwonGroup}' --cptrigger='${cpTrigger}' --name='${name}' --template='${template}'`);
                _log('verbose', `gulp setup ran with arguments`);
                exec.execSync(`mv _env.php .env.php`);
                _log('verbose', `reanamed .env.php`);
                ...
                _log('verbose', `symlinked scripts/.env.sh to Craft Scripts`);
                ...
                _verboseExec(`npm run prod`);
                _log('verbose', `WB Starter production script ran (npm run prod)`);
...
function _verboseExec(command) {
    if (verbose) {
        ...
        exec.execSync(`${command} &> /dev/null`);
    }
}
