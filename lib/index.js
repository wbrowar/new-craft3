...
function run() {
    ...
    inquirer.prompt(questions).then(function (answers) {
        name = answers['clientCode'] + '-' + answers['projectName'];
        ...
        if (!fs.existsSync(projectDir)){
            ...
            _download('https://github.com/wbrowar/WB-Starter/archive/master.zip', () => {
                ...
                exec.spawnSync(`npm run setup --name=${name} --gitorg=${gitorg}`, [], { stdio: 'inherit', shell: true });

                console.log(chalk.blue.bold('[ Running Composer Install ]'));
                exec.spawnSync(`composer install --ignore-platform-reqs`, [], { stdio: 'inherit', shell: true });

                console.log(chalk.blue.bold('[ Symlink Craft Scripts .env File ]'));
                exec.execSync(`cp scripts/craft3-example.env.sh scripts/.env`);
                exec.execSync(`ln -s scripts/.env vendor/nystudio107/craft-scripts/scripts/.env`);

                console.log(chalk.blue.bold('[ Move Setup Files ]'));
